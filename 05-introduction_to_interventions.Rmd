...
The ladder of causality has 3 levels: 1) Association 2) Intervention 3) Counterfactual.

```{r fig1, fig.cap = "figure caption.", fig.height=7, fig.width=7 ,fig.align='center'}
library(png)
library(grid)
...
one event is associated with another if observing one changes the likelihood of observing the other(from the book of why by Pearl).
...
* A perfect intervention is to artificially assign the value of a random variable. This means that once an intervention is performed on that random variable, its value will be deterministic.
...
```{r fig2, fig.cap = "figure caption.", fig.height=4, fig.width=4 ,fig.align='center'}
library(png)
library(grid)
...
```{r fig3, fig.cap = "figure caption.", fig.height=4, fig.width=4 ,fig.align='center'}
library(png)
library(grid)
...
* There are other kinds of interventions 

  Soft interventions - A soft intervention is just a method in which an intervention increases the likelihood of entering the desired state. There are different examples of soft intervention 
...
* Interventions as Pearl's Do-calculus 
  ...
  * One must note that there is a significant difference between $P(Y|X)$ and $P(Y|do(X)$
  
  $P(Y|X)$ is an example of Association and the data is generated by performing rejection sampling until the desired number of samples is retrieved. There is no change in the original distribution of any of the random variables.
  
  However, $P(Y|do(X)$ is an intervention where the value of X is deterministic and becomes independent of its parents. Therefore, the resultant data from $P(Y|X)$ and $P(Y|do(X)$ is different.
  
* `do` operator as metaprogramming Pyro
* Advantages of the `do`-operater
  * Consider that it allows us to reason about interventions, when it is not feasible to do the intervention
  
  * That is why they are called "perfect" or "ideal"
  
  * The power of the do-operator is calculating the effect on an intervention without actually doing anything. The effect is calculated theoretically.
  
* There is a lot of controversy associated with the do-operator. Paul Holland firmly believes that "There is no causation without intervention". This implies that we cannot say that obesity is the cause of heart attack since obesity cannot be meaningfully intervened (i.e. Obesity can't be set individually without perturbing other factors that leads to obesity). Only factors that can be meaningfully intervened qualify as a cause of a given outcome.
